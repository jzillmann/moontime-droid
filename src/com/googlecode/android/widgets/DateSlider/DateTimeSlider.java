...
public class DateTimeSlider extends DateSlider {
  ...
  /**
   * Create the month, day and the timescroller and feed them with their
   * labelers and place them on the layout.
   */
  @Override
  public void onCreate(Bundle savedInstanceState) {
    // this needs to be called before everything else to set up the main layout
    // of the DateSlider
    ...
    // create the minute scroller and assign its labeler and add it to the
    // layout
    ...
    // this method _has_ to be called to set the onScrollListeners for all the
    // Scrollers
  ...
  // the day labeler takes care of providing each TimeTextView element in the
  // dayScroller
  // with the right label and information about its time representation
  protected Labeler dayLabeler = new Labeler() {

    /**
     * add "val" days to the month object that contains "time" and returns the
     * new TimeObject
     */
    @Override
    public TimeObject add(long time, int val) {
  ...
  protected void setTitle() {
    if (mTitleText != null) {
      ...
      mTitleText.setText(String.format("Selected DateTime: %te/%tm/%ty %tH:%02d", mTime, mTime, mTime, mTime, minute));
    }
  }
}
