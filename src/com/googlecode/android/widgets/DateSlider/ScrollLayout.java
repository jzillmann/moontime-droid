...
public class ScrollLayout extends LinearLayout {
  ...
  /**
   * This method is called usually after a ScrollLayout is instantiated, it
   * provides the scroller with all necessary information
   * 
   * @param labeler
   *          the labeler instance which will provide the ScrollLayout with time
   *          unit information
   * @param time
  ...
  public void setLabeler(Labeler labeler, long time, int objwidth, int objheight) {
    ...
    // TODO: make it not dependent on the display width but rather on the layout
    // width
  ...
  public void setTime(long time, int loops) {
    ...
    if (loops > 2 || currelem.getStartTime() <= time && currelem.getEndTime() >= time) {
      if (loops > 2) {
        Log.d(TAG,
            String.format("time: %d, start: %d, end: %d", time, currelem.getStartTime(), currelem.getStartTime()));
        return;
      }
  ...
  /**
   * core scroll function which will replace and move TimeTextViews so that they
   * don't get scrolled out of the layout
   * 
   * @param x
   * @param y
   * @param notify
  ...
  protected void reScrollTo(int x, int y, boolean notify) {
    if (getChildCount() > 0) {
      ...
      if (mFirstElemOffset - mInitialOffset > objWidth / 2) {
        ...
        mFirstElemOffset = ((mFirstElemOffset - mInitialOffset - objWidth / 2) % objWidth) + mInitialOffset - objWidth
            / 2;
   ...
   * that there happens an endless scrolling with a very limited amount of views
   * 
   * @param steps
   */
